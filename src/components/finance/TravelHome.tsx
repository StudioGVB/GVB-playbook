...
function PalmTreeWatermark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 100 100" fill="currentColor" className={className}>
      <path d="M50 90 Q 48 50 40 25 C 42 22 58 22 60 25 Q 52 50 50 90 Z" />
      <path d="M40 25 C 25 15 5 20 0 35 C 15 35 30 30 40 25 Z" />
      <path d="M60 25 C 75 15 95 20 100 35 C 85 35 70 30 60 25 Z" />
      <path d="M45 22 C 35 5 15 0 0 5 C 10 15 25 20 45 22 Z" />
      <path d="M55 22 C 65 5 85 0 100 5 C 90 15 75 20 55 22 Z" />
      <path d="M50 20 C 50 0 50 -10 50 -10 C 45 5 45 15 50 20 Z" />
    </svg>
  );
}
...
function SnowflakeWatermark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" className={className}>
      ...
      <polygon points="12 6 10 4 14 4" fill="currentColor" />
      <polygon points="12 18 10 20 14 20" fill="currentColor" />
      <polygon points="6 12 4 10 4 14" fill="currentColor" />
      <polygon points="18 12 20 10 20 14" fill="currentColor" />
    </svg>
  );
}
...
export function TravelHome({ trip, goal, transactions, categories = [], onExitToBudget }: TravelHomeProps) {
  ...
  const isWinterTheme = useMemo(() => {
    const text = (trip.name + ' ' + (trip.notes || '')).toLowerCase();
    return /winter|snow|ski|alps|iceland|lapland|chamonix|zermatt/i.test(text);
  }, [trip]);
      ...
      <div className="relative z-10 space-y-4">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <LogTripExpenseDialog
            ...
            trigger={
              <Button size="sm" className="gap-1.5 bg-sky-500 hover:bg-sky-600 text-white font-bold shadow-xs">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="flex-1 bg-card/90" onClick={() => navigate('/travel/trips')}>
            Manage Trip & Checklist
          </Button>
          <Button variant="outline" size="sm" className="flex-1 bg-card/90" onClick={() => navigate('/finance/transactions')}>
            All Transactions
          </Button>
          <Button variant="outline" size="sm" className="flex-1 bg-card/90" onClick={onExitToBudget}>
            Standard Budget
          </Button>
        </div>
      </div>
